...
WORK = ROOT.parent / ".dogg-markets-rapp1-v2-test-work"
NATIVE_PINS = {
    ...
    "tools/collect.py": "9676194f268e94c8bbc561c8041158e3a2ca1bd92b7d27a8db803511479fd501",
...
class Bridge2(unittest.TestCase):
    def setUp(self):
        ...
        self.world = self.make_world(self.ticks)
        ...
        self.write_chain("markets", self.world)
        ...
        initialized = B.initialize(
            ...
            "markets", B.CANONICAL_REF, "local", "dogg-markets-bridge",
    ...
    def make_world(self, ticks, note="markets"):
        frames = []
        for seq, tick in enumerate(ticks):
            ...
            head = frames[-1] if frames else None
            frames.append(B.NATIVE_RAPP.build_frame(
                "markets.snapshot", "markets:@kody-w/dogg-markets", seq,
    ...
    def extend_native(self):
        ...
        head = self.world[-1]
        self.world.append(B.NATIVE_RAPP.build_frame(
            "markets.snapshot", "markets:@kody-w/dogg-markets", seq,
            f"2026-09-17T00:01:{seq:02d}.000Z",
            {"tick": seq, "tick_frame": tick["frame_hash"], "note": f"world-{seq}"},
            head["payload_hash"],
        ))
        self.write_chain("ticks", self.ticks, repo=self.tick_repo)
    ...
    def test_cli_ref_feature_branch_and_origin_alias_refuse(self):
        ...
        with self.assertRaisesRegex(ValueError, "exact Git top-level"):
            B.check_git_source(
                self.repo / "markets", self.revision, B.CANONICAL_REF
            )
        with self.assertRaisesRegex(ValueError, "exact Git top-level"):
            B.append_paths(
                self.repo / "markets", self.tick_repo, self.state,
                [self.repo / "output"],
            )
    ...
    def test_every_historical_frame_rechecks_market_and_tick_commits(self):
        self.append()
        expected = []
        for seq in range(3):
            frame = json.loads(
    ...
    def test_append_idempotent_retry_and_candidate_bytes_reused(self):
        native_before = {
            ...
            for root, chain in ((self.repo, "markets"), (self.tick_repo, "ticks"))
        ...
        self.ticks.append(tick)
        head = self.world[-1]
        world = B.NATIVE_RAPP.build_frame(
            "markets.snapshot", "markets:@kody-w/dogg-markets", 3,
            "2026-09-17T00:01:03.000Z",
            {"tick": 3, "tick_frame": tick["frame_hash"], "note": "world-3"},
            head["payload_hash"],
        )
    ...
    def test_accepted_controller_frame_and_source_metadata_are_immutable(self):
        ...
        changed_payload["native_source"]["storage_path"] = "markets/1.json"
    ...
    def test_outputs_are_disjoint_before_any_write(self):
        ...
        with self.assertRaisesRegex(ValueError, "state and registry output"):
            B.initialize(
                self.repo, self.revision, self.tick_repo, self.tick_revision,
                "markets", B.CANONICAL_REF, "local", "overlap",
    ...
    def test_pending_recovery_survives_new_source_and_removes_abandoned_head(self):
        ...
        abandoned = {
            **current,
            ...
            "native_seq": 5,
            "native_frame_hash": self.world[5]["frame_hash"],
    ...
    def test_native_commit_path_raw_rollback_fork_and_genesis_refuse(self):
        ...
        world2 = self.repo / "markets" / "2.json"
        original = world2.read_bytes()
        world2.write_bytes(original.replace(b"world-2", b"changed"))
        ...
        self.write_chain("markets", self.world[:2])
        ...
        forked = copy.deepcopy(self.world)
        forked[2] = B.NATIVE_RAPP.build_frame(
            "markets.snapshot", "markets:@kody-w/dogg-markets", 2, forked[2]["utc"],
            {**forked[2]["payload"], "note": "fork"},
            forked[1]["payload_hash"],
        )
        self.write_chain("markets", forked)
        ...
        changed = self.make_world(self.ticks, note="new-genesis")
        self.write_chain("markets", changed)
    ...
    def test_every_accepted_frame_has_exact_historical_native_provenance(self):
        copied = self.repo / "markets" / "historical-copy.json"
        copied.write_bytes((self.repo / "markets" / "1.json").read_bytes())
        self.commit("add misleading historical native copy")
        self.append()

        def use_copy(kind, payload):
            payload["native_source"]["storage_path"] = "markets/historical-copy.json"
            return kind, payload
    ...
    def test_source_bytes_and_modes_are_bound_to_the_commit_tree(self):
        self.write_chain("markets", self.world, epoch_size=2, sealed_epochs=1)
        committed = self.commit("seal source for immutable tree test")
        committed_epoch = B.git_blob(
            self.repo, committed, "markets/epochs/0.jsonl"
        )
        ...
        def mutate_after_git_check(*args, **kwargs):
            ...
            if not mutated:
                mutated = True
                epoch = self.repo / "markets" / "epochs" / "0.jsonl"
                epoch.write_bytes(epoch.read_bytes().replace(b"world-1", b"mutable"))
                (self.repo / "markets" / "2.json").write_bytes(b"not native JSON\n")
            return result
        ...
        self.git("checkout", "--", "markets")
        os.chmod(self.repo / "markets" / "2.json", 0o755)
        mode_commit = self.commit("make native source executable")
        self.assertIn(
            "100755",
            self.git("ls-tree", mode_commit, "markets/2.json"),
        )
    ...
    def test_sealed_epoch_source_is_exact_opaque_line(self):
        self.write_chain("markets", self.world, epoch_size=2, sealed_epochs=1)
        self.commit("seal native markets epoch")
        report = self.append()
        self.assertEqual(report["frames_verified"], 3)
        frame1 = json.loads((self.output / "frames" / "1.json").read_bytes())
        source = frame1["payload"]["native_source"]
        self.assertEqual(source["storage_path"], "markets/epochs/0.jsonl")
    ...
    def test_sealed_epoch_over_one_mib_is_valid_when_each_record_is_bounded(self):
        ...
        self.write_chain("markets", frames, epoch_size=3, sealed_epochs=1)
        epoch = self.repo / "markets" / "epochs" / "0.jsonl"
        ...
        for case in cases:
            with self.subTest(case=case):
                ...
                if case == "aggregate":
                    ...
                    head_path = self.repo / "markets" / "HEAD.json"
                    ...
                    shutil.rmtree(self.repo / "markets" / "epochs")
                    for seq, frame in enumerate(self.world):
                        (self.repo / "markets" / f"{seq}.json").write_text(
    ...
    def test_sealed_epoch_structure_and_encoding_refuse(self):
        ...
        cases = {
            ...
            "order": ("native seq gap at markets/0", lambda lines: list(reversed(lines))),
    ...
    def test_wrong_or_stale_native_tick_frame_refuses(self):
        ...
        changed[1] = B.NATIVE_RAPP.build_frame(
            "markets.snapshot", "markets:@kody-w/dogg-markets", 1, changed[1]["utc"],
            {"tick": 1, "tick_frame": self.ticks[0]["frame_hash"], "note": "stale"},
            changed[0]["payload_hash"],
        )
        changed[2] = B.NATIVE_RAPP.build_frame(
            "markets.snapshot", "markets:@kody-w/dogg-markets", 2, changed[2]["utc"],
    ...
    def test_tick_anchor_stream_kind_and_payload_are_exact_everywhere(self):
        ...
        cases = (
            ...
            ("kind", "tick:@kody-w/global", "markets.snapshot", {"tick": 0}, "wrong kind"),
        ...
        native_source = self.source_tuple(
            native, "markets", native_raw, B.SOURCE_REPOSITORY
        )
