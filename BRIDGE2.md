# Native `markets:@kody-w/dogg-markets` → RAPP/1

`dogg-rapp1-bridge/2` is a subordinate operational profile over frozen RAPP/1.
It continuously projects the existing native DOGG/0 `markets/` chain into a
...
|---|---|---|
| Market records | `https://github.com/kody-w/dogg-markets.git` | `markets/` |
...
Every market frame's actual `payload.tick_frame` must resolve at
...
the markets repository as tick authority.
...
exclude the bundle's LF delimiter. For every market projection `tick_frame` is
...
RAPP sequence `N` projects market sequence `N`. Cold start projects genesis
through the verified market head; later runs append only the missing suffix.
Before any further minting, every retained frame reverifies both its historical
market commit and its historical tick commit, both complete native chains,
...
```json
{"path":"<safe relative path>","ref":"refs/remotes/origin/main","repository":"https://github.com/kody-w/dogg-markets.git"}
```
...
3. this exact profile path and raw digest in the markets repository;
...
protected-main checkpoint in the markets repository. The checkpoint and market
source commit must both be on canonical main, and the checkpoint must be an
ancestor of that market source commit. The estate-owner RAPPID must arrive
...
```sh
MARKETS_COMMIT=$(GIT_NO_REPLACE_OBJECTS=1 git --no-replace-objects rev-parse HEAD)
...
python3 tools/rapp1_bridge_v2.py generate-key \
  ...
  --out "$HOME/.dogg-private/dogg-markets-rapp1-bridge.pem"

python3 tools/rapp1_bridge_v2.py init \
  --source-root . --source-revision "$MARKETS_COMMIT" \
  --tick-source-root "$TICK_ROOT" --tick-source-commit "$TICK_COMMIT" \
  --main-ref refs/remotes/origin/main --chain markets \
  --owner kody-w --slug dogg-markets-rapp1-bridge \
  --canonical-registry-path rapp1-bridge-v2/registry.json \
  --state "$HOME/.dogg-private/markets-bridge-v2-state" \
  --registry-out "$HOME/.dogg-private/markets-registry-0.json" \
  --signing-key "$HOME/.dogg-private/dogg-markets-rapp1-bridge.pem"
```
...
5. choose and monitor immutable canonical-main market and tick commits for each
   append.
...
native writer identity or the truth of market observations. Bridge signatures
